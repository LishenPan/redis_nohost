...
//			printf("find req\n");
		...
		if ( (req = master_GetRequest(rdata->fd, rdata->buf, rdata->nread, req)) != (req_t*)0 ) {
//			printf("split\n");
...
void* master_handler(void* arg) {
	...
	serv_adr.sin_port = htons(atoi((char*)arg));
	serv_adr.sin_addr.s_addr = inet_addr("10.0.0.4");
	...
	while(1) {
		fd = 0;
		if ( (ret = recv(serv_sock, buffer, BUFSIZE, 0)) <= 0 ) {
			if ( ret == 0 ) {
				printf("byebye\n");
				free(buffer);
				close(serv_sock);
		...
		fd += buffer[3] - '0';
		type = (buffer[4] - '0') * 10 + buffer[5] - '0';
		switch(type) {
		case 1 :
			master_SendOkCommand(fd);
			break;
		case 2 : 
			for(i = 0; i < 3; i++) {
				value_len += buffer[6+i] - '0';
				value_len *= 10;
			}
			value_len += buffer[9];
			master_SendBulkValue(fd, &buffer[10], value_len);
			break;
		case 3 :
			break;
		default :
...
//	lr_inter_init();	
	...
	pthread_detach(t_id);
	pthread_create(&t_id, NULL, master_handler, (void*)argv[2]);
	pthread_detach(t_id);
